...
int main(int argc, char *argv[])
{
    ...
    if (rank == root)
    {
        // root uzima sebi prvih q elemenata
        for (int i = 0; i < q; i++)
            local_b[i] = b[i];
        ...
        for (int p = 1; p < size; p++)
        {
            for (int i = 0; i < q; i++)
                tmp_b[i] = b[i + p * q];
            MPI_Send(tmp_b, q, MPI_INT, p, 0, MPI_COMM_WORLD);
        }
    }
    else
        MPI_Recv(local_b, q, MPI_INT, root, 0, MPI_COMM_WORLD, &status);
    ...
    // svi salju root-u, root traži maksimum
    // MPI_Reduce(&in, &out, 1, MPI_2INT, MPI_MAXLOC, root, MPI_COMM_WORLD);
    if (rank != root)
        MPI_Send(&in, 1, MPI_2INT, root, 0, MPI_COMM_WORLD);
    else
    {
        // root uzima svoje kao pocetne vrednosti
        out = in;
        for (int p = 1; p < size; p++)
        {
            // prima direktno u 'in' pri svakom Recv i poredi sa 'out'
            // nema potrebe za pomocnom promenljivom
    ...
    // sabiranje parcijalnih suma svake vrste, u localrowsum[0] ce biti sume svake vrste
    // MPI_Reduce(row_sum, local_row_sum, k, MPI_INT, MPI_SUM, out.rank, MPI_COMM_WORLD);
    if (rank != out.rank)
