...
int main(int argc, char *argv[])
{
    ...
    // ── PETLJA 1: maksimum lokalnog A (za MAXLOC) ──
    for (int i = 0; i < m; i++)
        for (int j = 0; j < k; j++)
            if (in.value < local_a[i][j])
                in.value = local_a[i][j];

    // ── PETLJA 2: glavni rezultat — redovi lokalne C ──
    for (int i = 0; i < m; i++)
        for (int j = 0; j < n; j++)
        {
            local_c[i][j] = 0;
    ...
    // ── PETLJA 3: lokalna suma kolona B (ciklično — SVI procesi učestvuju) ──
    for (int j = 0; j < n; j++)
        local_col_sum[j] = 0;
    for (int i = rank; i < k; i += size)
        for (int j = 0; j < n; j++)
            local_col_sum[j] += b[i][j];
    ...
    // MPI_Reduce(local_col_sum, col_sum, n, MPI_SUM, out.rank)
    if (rank != out.rank)
    {
        MPI_Send(local_col_sum, n, MPI_INT, out.rank, 0, MPI_COMM_WORLD);
    }
    else
    {
        for (int j = 0; j < n; j++)
            col_sum[j] = local_col_sum[j];
        for (int p = 0; p < size; p++)
        {
            if (p == out.rank)
                continue;
            MPI_Recv(local_col_sum, n, MPI_INT, p, 0, MPI_COMM_WORLD, &status);
            for (int j = 0; j < n; j++)
                col_sum[j] += local_col_sum[j];
        }
    }
