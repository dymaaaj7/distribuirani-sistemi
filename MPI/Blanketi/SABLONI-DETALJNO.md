...
- **Oktobar 2025 b** = **Jun 2026 a** — varijanta sa **običnim nizom** umesto matrice; u kompaktnoj verziji izdvojeno kao **Tip 6** (v. napomenu ispod)
...
> **Varijanta — Oktobar 2025 b = Jun 2026 a (niz umesto matrice; = Tip 6 u kompaktnoj verziji):** isti skelet kao Tip 2, samo što se umesto kolona matrice deli **niz** blok raspodelom (`MPI_Scatter` po `k = N/size` elemenata). Formula: `R = Σ(ā+aᵢ)/(b+c)`, gde je `ā` srednja vrednost niza. Novine u odnosu na standardni Tip 2:
...
| Oktobar 2025 b | Tip 6 | Scatter niza → MAXLOC (b i c inicijalizuje proces sa maksimumom) → MINLOC (broj prostih) → Reduce SUM u procesu sa najmanje prostih; formula `R = Σ(ā+aᵢ)/(b+c)` |
| Jun 2026 a | Tip 6 | **Identičan** Oktobru 2025 b — isti zadatak sa `R = Σ(ā+aᵢ)/(b+c)` |
| Jun 2026 b | Jedinstven (teorija) | Svaki proces ima `b1[2]`, svi treba da dobiju `b2[8]` sa svim elementima: `MPI_Gather(b1,2,...,root)` + `MPI_Bcast(b2,8,...,root)` — traži se minimalan broj poziva |
